...
let jsRe = /if\s*\([^)]+\s*&&\s*Array\.isArray[^)]+\)\s*\{[\s\S]*?else\s*if\s*\([^)]+\)\s*\{[\s\S]*?\`\s*;[\s]*\}/;
let r3 = `let items = [];
...
content = content.replace(/\/\*\s*Mobile Optimization\s*\*\/[\s\S]*?\}\s*\/\*\s*Mobile Optimization\s*\*\/[\s\S]*?\}/, `/* Mobile Optimization */
...
console.log("Spot included?", content.includes("clamp(20px, 3.5vw, 35px)"));
console.log("JS included?", content.includes("makeMultilineRow"));
